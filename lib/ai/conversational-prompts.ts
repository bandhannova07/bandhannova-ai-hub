// Conversational AI System Prompts
// Optimized for natural, engaging, and smart conversations

import { getPromptCache } from './prompt-cache-manager';

/**
 * Get conversational system prompt based on mode
 * Uses server-side cache for instant reuse
 */
export function getConversationalPrompt(mode: 'quick' | 'normal' | 'thinking'): string {
    const cache = getPromptCache();
    const cacheKey = `conversational-${mode}`;
...
/**
 * Build complete conversational prompt
 */
export function buildConversationalPrompt(
    mode: 'quick' | 'normal' | 'thinking',
    userMessage: string
): string {
    return getConversationalPrompt(mode);
}
